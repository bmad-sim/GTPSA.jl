...
Truncated Power Series Algebra (TPSA) performs forward-mode automatic differentation (AD) similar to the dual-number implementation of `ForwardDiff.jl`. However, instead of performing nested derivatives for higher orders, TPSA naturally extends to arbitary orders by directly using the power series expansions. GTPSA also utilizes a fast monomial indexing function for propagating the partial derivatives. `GTPSA.jl` therefore has nearly the same performance as `ForwardDiff.jl` to first-order, but is significantly faster for second-order calculations and above.

GTPSA provides several advantages over current Julia AD packages:

1. **Speed**: By directly using the power series expansions, `GTPSA.jl` is significantly faster than `ForwardDiff.jl` for second-order and above, and has the roughly the same performance as `ForwardDiff.jl` for first-order calculations
2. **Custom Orders in Individual Variables**: For example, computing the Taylor expansion of $f(x_1,x_2)$ to 2nd order in $x_1$ and 1st order in $x_2$ is done trivially in GTPSA
...
**GTPSA is fast!** See the `benchmark/taylormap.jl` example for a speed comparison of `GTPSA.jl` with `ForwardDiff.jl` in calculating the Jacobian and Hessians for a system with 56 inputs and 4 outputs.
