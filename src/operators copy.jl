...
atan(t1::TPS{Float64,D}, a::Real)     = (t = TPS{Float64,D}(a); atan!(t, t1, t);  return t)
atan(a::Real, t1::TPS{Float64})     = (t = TPS{Float64,D}(a); atan!(t, t, t1);  return t)

# --- Unary functions that return TPS{Float64} ---
for t = (:real,  :imag, :angle, :abs)
@eval begin
($t)(t1::TPS{T,D}) where {T,D} = (t = TPS{real(T),D}(); $(Symbol(t,:!))(t, t1); return t)
end
end

# --- Unary functions that return TPS{ComplexF64} --- 
polar(t1::TPS{T,D}) where {T,D} = (t=TPS{ComplexF64,D}(); polar!(t,t1); return t)
complex(t1::TPS{T,D}) where {T,D} = TPS{ComplexF64,D}(t1)
