function format(t::NewTPS; coloffset=0, max_nn=-1)
  nv = numvars(t)
  np = numparams(t)
  nn = numnn(t)
...
show(io::IO, m::Vector{<:NewTPS}) = show_vec(io, m)
show(io::IO, ::MIME"text/plain", m::Vector{<:NewTPS}) = show_vec(io, m)

function show_vec(io::IO, m::Vector{<:NewTPS})
  N = length(m)
...
# Same for varnames!
function show_map!(io::IO, m::Vector{<:NewTPS}, lines_used::Ref=Ref{Int}(0), only_vars=false, varnames=1:length(m))
