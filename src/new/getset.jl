# --- Setters ---
# All
function setindex!(t::NewTPS, v::Union{AbstractVector{<:Number},Number}, idx::Union{TPSIndexType,AbstractVector{<:Integer},Nothing}=nothing; param::Union{Integer,Nothing}=nothing, params::Union{SMIndexType,Nothing}=nothing)
  lowset!(t, v, idx, param, params)
end

# Flat index
function lowset!(t::NewTPS, v::Number, i::Union{Nothing,Integer}, param::Union{Nothing,Integer}, params::Nothing)
...
function lowset!(t::NewTPS, v::AbstractVector{<:Number}, idxs::AbstractVector{<:Integer}, param::Nothing, params::Nothing)
...
# Monomial
function lowset!(t::NewTPS, v::Number, ords::MIndexType, param::Nothing, params::Nothing)
  setm!(t, length(ords), collect(Cuchar, ords), 0, v)
end

# By sparse monomial
function lowset!(t::NewTPS, v::Number, vars::SMIndexType, param::Nothing, params::Union{SMIndexType,Nothing})
  sm, n = pairs_to_sm(t, vars, params=params)
  setsm!(t, n, sm, 0, v)
end

# Sparse monomial with nothing for vars
function lowset!(t::NewTPS, v::Number, vars::Nothing, param::Nothing, params::SMIndexType)
...
# --- Getters ---
# All
function getindex(t::NewTPS, idx::Union{AbstractVector{<:Integer},TPSIndexType,Nothing}=nothing; param::Union{Integer,Nothing}=nothing, params::Union{SMIndexType,Nothing}=nothing)
  return lowget(t, idx, param, params)
end

# To override Base number.jl
getindex(t::NewTPS, idx::Integer) = lowget(t, idx, nothing, nothing)

# Flat index
function lowget(t::NewTPS, i::Union{Nothing,Integer}, param::Union{Nothing,Integer}, params::Nothing)
  if isnothing(i) && isnothing(param)
    return t
  elseif !isnothing(i) && !isnothing(param)
    error("Invalid monomial index specified. Please use ONE of variable/parameter index, index by order, or index by sparse monomial.")
...
# Flat index vectorized
function lowget(t::NewTPS, idxs::AbstractVector{<:Integer}, param::Nothing, params::Nothing)
...
# Monomial
function lowget(t::NewTPS, ords::MIndexType, param::Nothing, params::Nothing)
  return getm(t, length(ords), collect(Cuchar, ords))
end

# By sparse monomial
function lowget(t::NewTPS, vars::SMIndexType, param::Nothing, params::Union{SMIndexType,Nothing})
  sm, n = pairs_to_sm(t, vars, params=params)
  return getsm(t, n, sm)
end

# Sparse monomial with nothing for vars
function lowget(t::NewTPS, vars::Nothing, param::Nothing, params::SMIndexType)
  sm, n = pairs_to_sm(t, Pair{Int,Int}[], params=params)
  return getsm(t, n, sm)
end
...
"""
numcoefs(t::NewTPS) = len(t)

firstindex(t::NewTPS) = 0
lastindex(t::NewTPS) = numcoefs(t)-1
...
function getindex(t::NewTPS, v::TPSColonIndexType; param::Union{Integer,Nothing}=nothing, params::Union{SMIndexType, Nothing}=nothing)
  if (v isa Vector)
...
# For flat indexing:
function getindex(t::NewTPS, v::Integer, c::Colon)
  par_mono = setup_mono(t, v, nothing, nothing)
  return slice(t, par_mono,false)
end

function getindex(t::NewTPS, c::Colon; param::Union{Integer,Nothing}=nothing)
  if isnothing(param)
    return t
  else
...
# --- par --- 
"""
    par(t::NewTPS, v::Union{TPSColonIndexType, Vector{Pair{<:Integer,<:Integer}}, Vector{<:Integer}, Integer, Colon, Nothing}=nothing; param::Union{Integer,Nothing}=nothing, params::Union{SMIndexType, Nothing}=nothing)
...
```
"""
function par(t::NewTPS, v::Union{TPSColonIndexType, Vector{Pair{<:Integer,<:Integer}}, Vector{<:Integer}, Integer, Colon, Nothing}=nothing; param::Union{Integer,Nothing}=nothing, params::Union{SMIndexType, Nothing}=nothing)
  if (v isa Vector)
    tmpv = tuple(v...)
...
# Flat index:
function setup_mono(t1::NewTPS, v::Integer, param::Nothing, params::Nothing)::Vector{Cuchar}
  par_mono = ones(Cuchar, v+1).*0xff
  par_mono[v] = 0x1
  return par_mono
end

function setup_mono(t1::NewTPS, v::Union{Nothing,Colon}, param::Integer, params::Nothing)::Vector{Cuchar}
...
function setup_mono(t1::NewTPS, v::MColonIndexType, param::Nothing, params::Nothing)::Vector{Cuchar}
...
function setup_mono(t1::NewTPS, v::SMColonIndexType, param::Nothing, params::SMIndexType)::Vector{Cuchar}
...
function setup_mono(t1::NewTPS, v::SMColonIndexType, param::Nothing, params::Nothing)::Vector{Cuchar}
  # Need to create array of orders with length nv + np
  ords, ___  = pairs_to_m(t1,filter(x->!(x isa Colon), v),zero_mono=false)
  return vcat(ords, 0xff)
end

function setup_mono(t1::NewTPS, v::Nothing, param::Nothing, params::SMIndexType)::Vector{Cuchar}
  # Need to create array of orders with length nv + np
...
function setup_mono(t1::NewTPS, v, param, params)
...
function slice(t1::NewTPS, par_mono::Vector{Cuchar}, par_it=true)
  nv = numvars(t1)
  np = numparams(t1)
  t = zero(t1)
  v = length(par_mono)-1
...
"""
    GTPSA.gradient!(result, t::NewTPS; include_params=false)
...
"""
function gradient!(result, t::NewTPS; include_params=false)
...
"""
    GTPSA.gradient(t::NewTPS; include_params=false)
...
"""
function gradient(t::NewTPS; include_params=false)
  n = numvars(t)
  if include_params
    n += numparams(t)
  end
...
"""
    GTPSA.jacobian!(result, m::AbstractVector{<:NewTPS}; include_params=false)
...
"""
function jacobian!(result, m::AbstractVector{<:NewTPS}; include_params=false)
...
"""
    GTPSA.jacobian(m::AbstractVector{<:NewTPS}; include_params=false)
...
- `J`              -- Jacobian of `m`
"""
function jacobian(m::AbstractVector{<:NewTPS}; include_params=false)
...
"""
    GTPSA.jacobiant!(result, m::AbstractVector{<:NewTPS}; include_params=false)
...
"""
function jacobiant!(result, m::AbstractVector{<:NewTPS}; include_params=false)
  Base.require_one_based_indexing(result, m)
  n = numvars(first(m))
  if include_params
    n += numparams(first(m))
...
"""
    GTPSA.jacobiant(m::AbstractVector{<:NewTPS}; include_params=false) where {N,P,I}
...
"""
function jacobiant(m::AbstractVector{<:NewTPS}; include_params=false)
  Base.require_one_based_indexing(m)
  n = numvars(first(m))
  if include_params
    n += numparams(first(m))
...
"""
    GTPSA.hessian!(result, t::NewTPS; include_params=false)
...
"""
function hessian!(result, t::NewTPS; include_params=false)
  Base.require_one_based_indexing(result)
...
"""
    GTPSA.hessian(t::NewTPS; include_params=false)
...
"""
function hessian(t::NewTPS; include_params=false)
  desc = unsafe_load(t.d)
  n = desc.nv
  if include_params
    n += desc.np
