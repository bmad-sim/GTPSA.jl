...
# --- Evaluate ---
mad_eval!(na, ma::Vector{NewTPS{Float64}}, nb, tb, tc) = mad_tpsa_eval!(Cint(na), ma, Cint(nb), convert(Vector{Float64},tb), tc)
mad_eval!(na, ma::Vector{NewTPS{ComplexF64}}, nb, tb, tc) = mad_ctpsa_eval!(Cint(na), ma, Cint(nb), convert(Vector{ComplexF64},tb), tc)
...
"""
function evaluate!(y::Vector{T}, F::Vector{NewTPS{T}}, x::Vector{<:Number}) where {T}
...
# --- Integral ---
"""
    integ!(t::NewTPS{T}, t1::NewTPS{T}, var::Integer=1) where {T}
    ∫!(t::NewTPS{T}, t1::NewTPS{T}, var::Integer=1) where {T}
...
monomials is not currently supported.
"""
integ(t1::NewTPS, var=1) = (t = zero(t1); integ!(t, t1, var); return t)
const ∫ = integ

# --- Derivative ---
# Low-level equivalent calls for TPS and ComplexTPS:
mad_deriv!( t1::NewTPS{Float64},    t::NewTPS{Float64},    var) = mad_tpsa_deriv!(t1, t, Cint(var))
mad_deriv!( t1::NewTPS{ComplexF64}, t::NewTPS{ComplexF64}, var) = mad_ctpsa_deriv!(t1, t, Cint(var))
mad_derivm!(t1::NewTPS{Float64},    t::NewTPS{Float64},    n, ords) = mad_tpsa_derivm!(t1, t, Cint(n), convert(Vector{UInt8},ords))
mad_derivm!(t1::NewTPS{ComplexF64}, t::NewTPS{ComplexF64}, n, ords) = mad_ctpsa_derivm!(t1, t, Cint(n), convert(Vector{UInt8}, ords))

"""
    deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::Union{TPSIndexType, Nothing}=nothing; param::Union{Integer,Nothing}=nothing, params::Union{SMIndexType, Nothing}=nothing) where {T}
    ∂!(t::NewTPS{T}, t1::NewTPS{T}, v::Union{TPSIndexType, Nothing}=nothing; param::Union{Integer,Nothing}=nothing, params::Union{SMIndexType, Nothing}=nothing) where {T}
...
"""
function deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::Union{TPSIndexType, Nothing}=nothing; param::Union{Integer,Nothing}=nothing, params::Union{SMIndexType, Nothing}=nothing) where {T}
  low_deriv!(t, t1, v, param, params)
end

# Variable/parameter:
low_deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::Integer, param::Nothing, params::Nothing) where {T} = mad_deriv!(t1, t, v)
low_deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::Nothing, param::Integer, params::Nothing) where {T} = mad_deriv!(t1, t, param+numvars(t))

# Default to first variable if nothing passed:
low_deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::Nothing, param::Nothing, params::Nothing) where {T} = low_deriv!(t1, 1, nothing, nothing)

# Monomial by order:
low_deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::MIndexType, param::Nothing, params::Nothing) where {T} = mad_derivm!(t1, t, length(v), v)

# Monomial by sparse monomial:
function low_deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::SMIndexType, param::Nothing, params::SMIndexType) where {T}
  # Need to create array of orders with length nv + np
  ords, n = pairs_to_m(t1,v,params=params)
  mad_derivm!(t1, t, n, ords)
end

function low_deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::SMIndexType, param::Nothing, params::Nothing) where {T}
  # Need to create array of orders with length nv + np
  ords, n = pairs_to_m(t1,v)
  mad_derivm!(t1, t, n, ords)
end

function low_deriv!(t::NewTPS{T}, t1::NewTPS{T}, v::Nothing, param::Nothing, params::SMIndexType) where {T}
  # Need to create array of orders with length nv + np
...
low_deriv(t::NewTPS{T}, t1::NewTPS{T}, v, param, params) where {T} = error("Invalid monomial specified. Please use ONE of variable/parameter index, index by order, or index by sparse monomial.")
...
"""
    getord!(t::NewTPS{T}, t1::NewTPS{T}, order::Integer) where {T}

Extracts one homogenous polynomial from `t1` of the given order and 
fills `t` with the result in-place.
"""
getord!(t::NewTPS{Float64},    t1::NewTPS{Float64},    order::Integer) = mad_tpsa_getord!(t1, t, UInt8(order))
getord!(t::NewTPS{ComplexF64}, t1::NewTPS{ComplexF64}, order::Integer) = mad_ctpsa_getord!(t1, t, UInt8(order))
...
"""
getord(t1::NewTPS, order::Integer) = (t = zero(t1); getord!(t, t1, order); return t)


# --- cutord ---
"""
    cutord!(t::NewTPS{T}, t1::NewTPS{T}, order::Integer) where {T<:NewTPS}
...
"""
cutord!(t::NewTPS{Float64},    t1::NewTPS{Float64},    order::Integer) = mad_tpsa_cutord!(t1, t, Cint(order))
cutord!(t::NewTPS{ComplexF64}, t1::NewTPS{ComplexF64}, order::Integer) = mad_ctpsa_cutord!(t1, t, Cint(order))
...
```
"""
cutord(t1::NewTPS, order::Integer) = (t = zero(t1); cutord!(t, t1, order); return t)
...
"""
clearord!(t::NewTPS{Float64},    ord::Integer) = mad_tpsa_clrord!(t, UInt8(ord))
clearord!(t::NewTPS{ComplexF64}, ord::Integer) = mad_ctpsa_clrord!(t, UInt8(ord))
...
"""
clearord(t1::NewTPS, order::Integer) = (t = zero(t1); copy!(t,t1); clearord!(t,order); return t)
...
# --- composition ---
mad_compose!(na, ma::Vector{NewTPS{Float64}},    nb, mb::Vector{NewTPS{Float64}},    mc::Vector{NewTPS{Float64}}) = mad_tpsa_compose!(Cint(na), ma, Cint(nb), mb, mc)
mad_compose!(na, ma::Vector{NewTPS{ComplexF64}}, nb, mb::Vector{NewTPS{ComplexF64}}, mc::Vector{NewTPS{ComplexF64}}) = mad_ctpsa_compose!(Cint(na), ma, Cint(nb), mb, mc)


"""
    compose!(m::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}, m2::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}, m1::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}; work_prom::Union{Nothing,Vector{NewTPS{ComplexF64}}}=nothing)compose!(m::Vector{NewTPS{<:Union{Float64,ComplexF64}}}, m2::Vector{NewTPS{<:Union{Float64,ComplexF64}}}, m1::Vector{NewTPS{<:Union{Float64,ComplexF64}}}; work_prom::Union{Nothing,Vector{NewTPS{ComplexF64}}}=nothing)
...
"""
function compose!(m::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}, m2::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}, m1::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}; work_prom::Union{Nothing,Vector{NewTPS{ComplexF64}}}=nothing)
  n = length(m)
  n2 = length(m2)
  n1 = length(m1)
  ...
  if eltype(m) != eltype(m1)  # Promoting m1
    if isnothing(work_prom)
      m1_prom = Vector{NewTPS{ComplexF64}}(undef, n1)
      for i=1:n1  # Allocate
        @inbounds m1_prom[i] = NewTPS{ComplexF64}(use=first(m))
      end
    else
  ...
  elseif eltype(m) != eltype(m2) # Promoting m2
    if isnothing(work_prom)
      m2_prom = Vector{NewTPS{ComplexF64}}(undef, n)
      for i=1:n  # Allocate
        @inbounds m2_prom[i] = NewTPS{ComplexF64}(use=first(m))
      end
    else
...
"""
    compose(m2::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}, m1::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}})

Composes the vector functions `m2 ∘ m1`
"""
function compose(m2::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}, m1::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}})
...
∘(m2::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}, m1::Vector{<:NewTPS{<:Union{Float64,ComplexF64}}}) = compose(m2, m1)

# --- translate ---
mad_translate!(na, ma::Vector{NewTPS{Float64}},    nb, tb, mc::Vector{NewTPS{Float64}})    = mad_tpsa_translate!(Cint(na), ma, Cint(nb), convert(Vector{Float64}, tb), mc)
mad_translate!(na, ma::Vector{NewTPS{ComplexF64}}, nb, tb, mc::Vector{NewTPS{ComplexF64}}) = mad_ctpsa_translate!(Cint(na), ma, Cint(nb), convert(Vector{ComplexF64}, tb), mc)


"""
    translate!(m::Vector{<:NewTPS{T}}, m1::Vector{<:NewTPS{T}}, x::Vector{<:Number}) where {T}

Fills `m` with the vector function equal to `m1` with its expansion point translated by `x`.
"""
function translate!(m::Vector{<:NewTPS{T}}, m1::Vector{<:NewTPS{T}}, x::Vector{<:Number}) where {T}
...
"""
    translate(m1::Vector{<:NewTPS{T}}, x::Vector{<:Number}) where {T}

returns a vector function equal to `m1` with its expansion point translated by `x`.
"""
function translate(m1::Vector{<:NewTPS{T}}, x::Vector{<:Number}) where {T}
  n = length(m1)
  m = Vector{NewTPS{T}}(undef, n)
  desc = getdesc(first(m1))
  for i=1:n
    @inbounds m[i] = NewTPS{T}(use=desc)
  end
  translate!(m, m1, x)
  return m
end